...
async function main() {
  ...
  for (const num of storeNums) {
    ...
    try {
      ...
      storeMap[num] = { name: city, sheetId: '' };
      continue;
    }
    ...
    storeMap[num] = { name: city, sheetId: newFile.id };
  ...
  // Write stores.js
  const storesJs = `// Auto-generated by migrate.js on ${new Date().toISOString()}
const STORES = ${JSON.stringify(storeMap, null, 2)};
...
`;
  ...
  console.log('\nstores.js written with all Sheet IDs.');
  console.log('\nMigration complete!');
  console.log(`Copied ${Object.values(storeMap).filter(s => s.sheetId).length} / ${storeNums.length} stores.`);
}
